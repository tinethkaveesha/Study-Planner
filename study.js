...
document.addEventListener('DOMContentLoaded', function() {
    ...
    navLinks.forEach(link => {
        ...
        if (currentPage.includes(href.replace('.html', '')) || 
            (href === './study.html' && currentPage.endsWith('/'))) {
...
class AuthManager {
    ...
    saveUser(user) {
        localStorage.setItem('study_planner_user', JSON.stringify(user));
...
function showAuthModal(mode = 'login') {
    ...
    if (mode === 'login') {
        ...
        document.getElementById('auth-title').textContent = 'Welcome to Study Planner';
